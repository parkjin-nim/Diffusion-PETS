"""Unified PyTorch metrics for unconditional time-series generation."""
...
def _balanced(real, fake, max_samples, seed):
    ...
    return np.asarray(real[real_idx], dtype=np.float32), np.asarray(fake[fake_idx], dtype=np.float32)
...
def discriminative_score(real, fake, device, seed=0, iterations=2000, batch_size=128, max_samples=4096):
    """Post-hoc GRU classification score: abs(test accuracy - 0.5)."""
    _seed(seed)
    real, fake = _balanced(real, fake, max_samples, seed)
    ...
    split = int(0.8 * len(order)); train_idx, test_idx = order[:split], order[split:]
    loader = DataLoader(TensorDataset(torch.from_numpy(data[train_idx]), torch.from_numpy(labels[train_idx])),
...
def predictive_score(real, fake, device, seed=0, iterations=5000, batch_size=128, max_samples=4096):
    """Train a GRU on synthetic data and report real-data one-step MAE."""
    _seed(seed)
    real, fake = _balanced(real, fake, max_samples, seed)
    input_dim = max(fake.shape[-1] - 1, 1)
    fake_x = torch.from_numpy(fake[:, :-1, :input_dim])
    fake_y = torch.from_numpy(fake[:, 1:, -1:])
    loader = DataLoader(TensorDataset(fake_x, fake_y), batch_size=min(batch_size, len(fake_x)), shuffle=True)
    ...
    model.eval()
    with torch.no_grad():
        prediction = model(torch.from_numpy(real[:, :-1, :input_dim]).to(device)).cpu().numpy()
    return float(np.mean(np.abs(prediction - real[:, 1:, -1:])))


def evaluate_generation(real, fake, gpu, seed, max_samples=4096):
    """Compute the four metrics used by the former experiment notebooks."""
    real, fake = _balanced(real, fake, max_samples, seed)
    device = torch.device(f"cuda:{gpu}" if torch.cuda.is_available() else "cpu")
    return {
        "context_fid": float(Context_FID(real, fake, device=gpu if torch.cuda.is_available() else "cpu")),
        "correlational_score": correlational_score(real, fake, max_samples=max_samples, seed=seed),
        "discriminative_score": discriminative_score(real, fake, device=device, seed=seed),
        "predictive_score": predictive_score(real, fake, device=device, seed=seed),
    }
