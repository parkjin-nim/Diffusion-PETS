...
class CustomDataset(Dataset):
    def __init__(
        ...
        if scaler_data_root is not None:
            _, self.scaler = self.read_data(scaler_data_root, self.name)
    ...
    def __getsamples(self, data, proportion, seed):
        ...
        if self.save2npy:
            # Conditional 샘플링에서는 train 데이터와 test 데이터가 모두 필요하기 때문에 둘 다 저장합니다. 
            # 무조건부 샘플링에서는 test 데이터가 필요하지 않지만, 코드의 일관성을 위해 둘 다 저장합니다.
            if 1 - proportion > 0: # 
    ...
    def read_data(filepath, name=''):
        ...
        # etth, ettm, etth2, ettm2, exchange_rate, national_illness, traffic, weather 데이터셋은 첫 번째 열이 타임스탬프이므로 이를 제거합니다. 
        # 다른 데이터셋에서는 첫 번째 열이 타임스탬프가 아니거나 이미 제거되어 있을 수 있으므로 이 작업을 수행하지 않습니다.
        #if name == 'etth':
        if name == 'etth' or name == 'ettm' or name == 'etth2' or name == 'ettm2' or name == 'exchange_rate' or name == 'national_illness' or name == 'traffic' or name == 'weather' or name =='geoje':
            df.drop(df.columns[0], axis=1, inplace=True)
        # if name=='geoje':
        #     df.drop(columns=['time'])

        data = df.values
        # default range of MinMaxScaler is (0, 1), which is what we want.
        scaler = MinMaxScaler()
